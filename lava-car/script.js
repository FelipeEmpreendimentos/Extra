...
const compareBefore=document.querySelector('#comparison-before');
const compareDivider=document.querySelector('#comparison-divider');
...
if(reducedMotion||!('IntersectionObserver' in window)){
  ...
  revealElements.forEach((el,index)=>{el.style.transitionDelay=`${Math.min(index%4,3)*45}ms`;observer.observe(el);});
}
...
const updateComparison=()=>{
  if(!compareRange||!compareBefore||!compareDivider) return;
...
form?.addEventListener('submit',event=>{
  ...
  if(submit){const original=submit.innerHTML;submit.disabled=true;submit.textContent='Agendamento preparado ✓';setTimeout(()=>{submit.disabled=false;submit.innerHTML=original;},2200);}
  if(formStatus) formStatus.textContent='Demonstração concluída. Em um site real, o atendimento seria encaminhado para o WhatsApp da empresa.';
  form.reset();
});